...
class Employee_GUI(Pilots,Sky_marshall,Janitor,Airhostess,Aviator):
    emp_main = Tk()
    emp_main.geometry('1500x900')
    emp_main.title('EMPLOYEE WINDOW')

    def Main(self):
        self.emp_frame = Frame(self.emp_main, height=1000 , width=1500,bg='white')
        self.emp_frame.place(x=0,y=0)
        self.emp_frame.tkraise()
        self.welcome_img = PhotoImage(file='logo.png')
        ...
        self.emp_label2.place(x=0,y=400)

        pilot_button = Button(self.emp_frame , command=self.pilot_records)
        pilot_button.place(x=10, y=315)
        pilot_img = PhotoImage(file='pilot.png')
        pilot_button.config(image=pilot_img)

        sky_marshall_button = Button(self.emp_frame , command=self.sky_marshall_records ,bg='white')
        sky_marshall_button.place(x=270,y=315)
        marshall_img = PhotoImage(file='marshall.png')
        sky_marshall_button.config(image=marshall_img)

        arihostess_button = Button(self.emp_frame , command=self.air_hostess_records ,bg='white')
        arihostess_button.place(x=530,y=315)
        hostess_img = PhotoImage(file='airhostess.png')
        arihostess_button.config(image=hostess_img)

        aviator_button = Button(self.emp_frame , command=self.aviator_records,bg='white')
        aviator_button.place(x=790,y=315)
        aviator_img = PhotoImage(file='2ndsem4.png')
        aviator_button.config(image=aviator_img)

        janitor_button = Button(self.emp_frame, command=self.janitor_records,bg='white')
        janitor_button.place(x=1050,y=315)
        janitor_img = PhotoImage(file='janitor.png')
        janitor_button.config(image=janitor_img)

        quit_button = Button(self.emp_frame, command=quit)
        quit_button.place(x=1310,y=315)
        quit_img = PhotoImage(file='quitbutton.png')
